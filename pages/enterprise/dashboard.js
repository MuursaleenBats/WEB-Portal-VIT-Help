...
const Dashboard = (props) => {
  ...
  const loadGraphData = async () => {
    ...
    const graphDDataGetResponse = await axios.get(`http://65.2.142.67:3001/data/totalOrgCasesByMonth/${org.Id}`,{
      params:{
        onlyAccepted: true
      }
    });
  ...
  const loadBarData = async () => {
    var org = JSON.parse(localStorage.getItem("vh-org"));
    const barDataGetResponse = await axios.get(`http://65.2.142.67:3001/data/solvedOrgCasesByMonth/${org.Id}`,{
      params:{
        onlyAccepted: true
      }
    });
